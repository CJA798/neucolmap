...
DiskLightGlue::DiskLightGlue(const Options& options)
    : options_(options) {
  // Initialize ONNX Runtime environment
  env_ = std::make_unique<Ort::Env>(ORT_LOGGING_LEVEL_WARNING, "DiskLightGlue");
  ...
  if (options_.use_gpu) {
    try {
      ...
      LOG(INFO) << "DiskLightGlue: Using GPU device " << options_.gpu_index;
  ...
  try {
    ...
    LOG(INFO) << "DiskLightGlue: Model loaded from " << options_.model_path;
  } catch (const Ort::Exception& e) {
    LOG(FATAL) << "Failed to load ONNX model: " << e.what();
  }
}

DiskLightGlue::~DiskLightGlue() = default;

std::vector<float> DiskLightGlue::PreprocessImage(const Bitmap& bitmap) {
  ...
  // DISK uses RGB, so keep all 3 channels (convert BGR to RGB)
  cv::Mat rgb;
  cv::cvtColor(img, rgb, cv::COLOR_BGR2RGB);
  
  // Resize to target size
  cv::Mat resized;
  cv::resize(rgb, resized, cv::Size(options_.image_size, options_.image_size));
  
  // Normalize to [0, 1]
  cv::Mat normalized;
  resized.convertTo(normalized, CV_32F, 1.0 / 255.0);
  
  // Convert to vector in CHW format: [C, H, W]
  std::vector<float> input_data(3 * options_.image_size * options_.image_size);
  for (int c = 0; c < 3; ++c) {
    for (int y = 0; y < options_.image_size; ++y) {
      for (int x = 0; x < options_.image_size; ++x) {
        input_data[c * options_.image_size * options_.image_size + y * options_.image_size + x] = 
            normalized.at<cv::Vec3f>(y, x)[c];
      }
    }
  }
...
bool DiskLightGlue::ExtractAndMatch(
  ...
  try {
    ...
    const int channels = 3;  // DISK uses RGB
    ...
    LOG(INFO) << StringPrintf(
        "DiskLightGlue: Extracted %d keypoints per image, found %d matches",
